...
var bshb = require('./bosch.js').bshb
...
module.exports = function (homebridge) {
  ...
  homebridge.registerAccessory("homebridge-bosch-energy-meter", "BoschEnergyMeter", EnergyMeter);
}
...
function EnergyMeter (log, config) {
  ...
  // this.url = "http://" + this.ip + "/rpc/shelly.getstatus";
...
EnergyMeter.prototype.updateState = function () {
  ...
  this.last_value = new Promise((resolve, reject) => {
    ...
    if (this.debug_log) { this.log('Requesting energy values from Shelly 3EM(EM) ...'); }
        ...
        bshb
        .getBshcClient()
        .getDeviceServices(this.deviceId, "PowerMeter")
        .subscribe(response => {
          const allPowerMeters = response.parsedResponse.filter(_ => _.state && _.state['@type'] === "powerMeterState")                        // TODO: do request with bosch sdk, and set values
                            this.powerConsumption = Math.abs((parseFloat(allPowerMeters[0].state.powerConsumption)));
              this.totalPowerConsumption = Math.abs((parseFloat(allPowerMeters[0].state.energyConsumption)/1000));

              resolve(this.powerConsumption,this.totalPowerConsumption)
      this.waiting_response = false;
